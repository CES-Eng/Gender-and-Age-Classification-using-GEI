...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, cache=True):
    ...
    if dataset == 'OUMVLP':
        for _vseq in sorted(list(os.listdir(dataset_path))):
            _seq_path = osp.join(dataset_path, _vseq)
            vseq = _vseq.replace("Silhouette_","")
            _view,_seq_type = vseq.split("-")
            seqs = os.listdir(_seq_path)
            for _label in seqs:
                _seq_dir = osp.join(_seq_path, _label)
            ...
            for gei in seqs:
                if len(seqs) > 0:
                    seq_dir.append([dataset_path])
                    label.append(gei.replace(".png",""))
                    seq_type.append(gei)
                    view.append(gei)
        ...
        for _label in sorted(list(os.listdir(dataset_path))):
            ...
            if (dataset == 'CASIA-B' or dataset == 'gei') and _label == '005':
                continue
